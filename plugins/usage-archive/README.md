...
- Node.js 22.11.0 以上（ccusage 20 の動作条件）
- その Node.js が、どのフォルダからでも `node` で起動できること

フックは会話を開いたプロジェクトのフォルダで `node` を実行します。mise や nvm などで
フォルダごとに Node.js を切り替えている場合、指定のないフォルダでは `node` が見つからず、
何も記録されません。全体の既定を設定してください。

    mise use -g node@24        # mise の場合
    nvm alias default 24       # nvm の場合
